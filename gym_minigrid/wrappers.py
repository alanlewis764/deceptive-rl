...
class FlatObsWrapper(gym.core.ObservationWrapper):
    """
    Remove the mission statement which is not relevant to our task
    """
    ...
    def observation(self, obs):
        """
        Compared to original, we remove the string mission statement since it is not relevant to our task
        """
        env = self.unwrapped
        full_grid = env.grid.encode()
...
from .minigrid import Goal, FakeGoal
...
class SimpleObsWrapper(gym.core.ObservationWrapper):
    ...
    def observation(self, obs):
        env = self.unwrapped
        # only give the agents location and direction as the state information
        state = (self.agent_pos[0], self.agent_pos[1], env.agent_dir)  # agent location and direction
        return state


class SimpleObsWrapperWithGoal(gym.core.ObservationWrapper):
    ...
    def observation(self, obs):
        env = self.unwrapped
        # only give the agents location and direction as the state information
        state = (self.agent_pos[0], self.agent_pos[1], env.agent_dir, self.goal_pos[0], self.goal_pos[1])  # agent location and direction
        return state